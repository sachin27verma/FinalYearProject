...
data_copy_df = pd.read_csv('tt.csv')
selected_urls_df = data_copy_df[data_copy_df['flag'] == 1]
urls = selected_urls_df['link'].tolist()
...
output_df.to_csv('scraped_data_true.csv', index=False, encoding='utf-8')
print("Scraping complete! Data saved to scraped_data_true.csv")

# Save failed URLs to a separate CSV file
if failed_urls:
    failed_df = pd.DataFrame(failed_urls, columns=['url'])
    failed_df.to_csv('failed_urls_true.csv', index=False)
